...
def MainProgram(input_image, color, output_image, opacity):
    ...
    try:      
        image_ext = input_image.split('/')[-1].split('.')[-1]
        image_name = input_image.split('/')[-1].split('.')[-2]
...
if __name__ == "__main__":
    ...
    if args.input_image is None:
        ...
        for img in img_list:
            ...
            # print(img_path)
            MainProgram(img_path, args.color, args.output, args.opacity)
    else:
